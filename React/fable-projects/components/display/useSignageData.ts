...
// - DISP-05: 表示項目設定（M-DISPLAY の config）も同時に取得
// - 時計表示用に now を1秒間隔で更新
export function useSignageData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [facilitiesResult, reservationsResult, configResult] =
      await Promise.all([
    ...
    if (facilitiesResult.error || reservationsResult.error) {
      ...
      setDisplayConfig(parseDisplayConfig(configResult.data?.config ?? null));
    }
    setLoading(false);
  }, []);
  ...
  return { facilities, reservations, displayConfig, now, loading, error };
}
